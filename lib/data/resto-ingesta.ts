...
export async function getRestoIngestaRecords(unitId: number | null, organizationId: number) {
  const rows = await db
    .select({
    ...
    .where(
      and(
        eq(restoIngestaRecords.organizationId, organizationId),
...
export async function getRestoIngestaMonthlySummary(
  unitId: number | null,
