          ...
          <div key={group.id} style={{ borderTop: "1px solid var(--border)", paddingTop: 12, marginTop: 12 }}>
            ...
            {group.items.map((item) => (
              <div key={item.id} style={{ display: "flex", gap: 8, marginBottom: 6, alignItems: "center" }}>
                <form action={updateAuditItem} style={{ display: "flex", gap: 8, flex: 1 }}>
                  <input type="hidden" name="id" value={item.id} />
                  <input name="label" defaultValue={item.label} aria-label="Pergunta" style={{ flex: 1 }} />
                  <button className="btn-small" type="submit">Salvar</button>
                </form>
                <DeleteButton action={deleteAuditItem} id={item.id} confirmText="Remover essa pergunta?" />
              </div>
            ))}
            <form action={addAuditItem} style={{ display: "flex", gap: 8, marginTop: 8 }}>
              <input type="hidden" name="groupId" value={group.id} />
              <input name="label" placeholder="Nova pergunta do grupo" style={{ flex: 1 }} />
